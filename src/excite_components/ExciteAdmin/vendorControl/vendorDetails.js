...
class ProcessVendor extends React.Component{
    ...
    Business_Profile_id = async (token) =>{
      ...
      userDetail = async(token)=>{
        const userID = this.props.match.params.vendorProfileID
        const endpoint = host + `/excite-admin-connect/user-data/${userID}`
        axios.defaults.headers = {
          "Content-Type": "application/json",
          Authorization: `Token ${token}`
        };
        ...
        .then(res =>{
          if (res.status == 200){
            this.setState({
              AffliteData:res.data['affliate_code']
            })
            console.log('Affliate Code',res.data['affliate_code'])
          } else{
    ...
    componentDidMount(){
        ...
        this.userDetail(this.props.token)
    ...
    componentWillReceiveProps(newProps) {
          if (newProps.token !== this.props.token) {
            if (newProps.token !== undefined && newProps.token !== null) {
                ...
                this.userDetail(newProps.token) 
            }
          }
        }
    ...
    render(){
        const {user_id, profile_id,   Business_Profile,
            business_profile_id,profile,AffliteData, 
            loading,
            error} = this.state
