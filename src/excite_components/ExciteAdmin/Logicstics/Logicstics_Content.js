...
class Admin_Logicstics_Details extends Component{
    ...
    Get_Order_Contents = async(token) =>{
        ...
        const endpoint = host + `/management/view_channel_content/${this.order_id}/`
        axios.get(endpoint)
        .then(res =>{
           this.setState({
    ...
    Change_Delivery_Status = async(checked)=>{
      ...
      const endpoint = host + `/management/change_delivery_status/${this.order_id}/`
    ...
    Delete_Order = async()=>{
      ...
      const endpoint = host + `/management/delete_logicstics/${this.order_id}/`
      await axios.post(endpoint)
      .then(res =>{
          openNotification(res.data['Message'])
      })
