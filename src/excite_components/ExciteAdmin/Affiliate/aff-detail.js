...
class AffDetail extends Component {
    ...
    getAffiliateDetail = (token) => {
      ...
      console.log(affiliateID);
        ...
        axios.get(`http://127.0.0.1:8000/excite-admin-connect/get-affiliate-codes/${affiliateID}/`)
        .then(res => {
          this.setState({
            aff_det: res.data[0]
          });
          const ref = this.state.aff_det.link 
          this.getAffiliateUsers(token, ref)
        });
    ...
    getAffiliateUsers = (token, ref) => {
      ...
      axios.get(`http://127.0.0.1:8000/excite-admin-connect/get-affiliate-users/${ref}/`)
      .then(res => {
        this.setState({
          user_list: res.data
        })
    ...
    componentDidMount(){
        if (this.props.token !== undefined && this.props.token !== null) {
          this.getAffiliateDetail(this.props.token)
        }
      }
      ...
      componentWillReceiveProps(newProps) {
        if (newProps.token !== this.props.token) {
          if (newProps.token !== undefined && newProps.token !== null) {
            this.getAffiliateDetail(newProps.token)
         }
        }
      }
